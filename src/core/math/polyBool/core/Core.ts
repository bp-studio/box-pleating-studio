...
export abstract class Core {
	...
	protected abstract mergeFill(ev: XEvent, eve: XEvent): void;

	/** calculate fill flags */
	protected abstract calculateFill(ev: XEvent, below: XEvent | null,
		polyInverted1: boolean, polyInverted2: boolean): void;
	...
	protected calculate(primaryPolyInverted: boolean, secondaryPolyInverted: boolean): Segment[] {
		...
		while(!this.events.isEmpty()) {
			let ev = this.events.getHead();

			if(ev.isStart) {

				let surrounding = status.findSurrounding(ev);
				let above = surrounding.before;
				let below = surrounding.after;
