...
@shrewd class SelectionController {
	...
	public process(event: paper.ToolEvent, ctrlKey?: boolean): void {
		...
		if(nowCtrl) {
			let p = SelectionController.controlPriority(nowCtrl);
			if(controls.some(c => SelectionController.controlPriority(c) < p)) {
	...
	@unorderedArray() private get _controls(): Control[] {
		...
		c.sort((a, b) => SelectionController.controlPriority(a) - SelectionController.controlPriority(b));
	 ...
	 */
	@shrewd private _updateDraggable() {
