...
namespace PolyBool {
	...
	function eventCompare(e1: Event, e2: Event): compare {
	...
	export class EventList extends List {
		...
		private add(ev: Event): void {
			this.findTransition(ev).insert(ev);
		}

		public addSegment(seg: Segment, primary: boolean): Event {
			let ev_start: Event = {
				isStart: true,
				pt: seg.start,
				seg,
		...
		private divide(ev: Event, pt: Point): Event {
		...
		public checkIntersection(ev1: Event, ev2: Event) {
			...
			if(i === false) {
				...
				if(i.alongA === 0) {
					...
					{ this.divide(ev1, b1); } else if(i.alongB === 0) // yes, somewhere between B's endpoints
					{ this.divide(ev1, i.pt); } else if(i.alongB === 1) // yes, at exactly b2
					{ this.divide(ev1, b2); }
				}
				...
				if(i.alongB === 0) {
					...
					{ this.divide(ev2, a1); } else if(i.alongA === 0) // yes, somewhere between A's endpoints (exclusive)
					{ this.divide(ev2, i.pt); } else if(i.alongA === 1) // yes, at exactly a2
					{ this.divide(ev2, a2); }
				}
			}
			return false;
		...
		public checkBothIntersections(ev: Event, above: Event | null, below: Event | null): false | Event {
			if(above) {
