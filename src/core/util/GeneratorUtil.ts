...
namespace GeneratorUtil {
	...
	export function* $first<T>(generators: Generator<T>[], filter: (value: T) => boolean): Generator<T> {
	...
	export function* $filter<T>(generator: Generator<T>, predicate: (value: T) => boolean): Generator<T> {
