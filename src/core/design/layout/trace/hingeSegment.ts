...
export interface HingeSegment extends Path {
	readonly $dir: QuadrantDirection;
}

/**
 * Break a hinge contour into segments by the given {@link SlashDirection}.
 */
export function createSegments(hinges: Path, dir: SlashDirection): HingeSegment[] {
	...
	for(let i = 0; i < l; i++) {
		...
		if(currentSegment) {
			...
			if(directions[index] != currentSegment.$dir) {
		...
		if(!currentSegment) {
			...
			if(nextDir % 2 == dir) {
				...
				(currentSegment as Writeable<HingeSegment>).$dir = nextDir;
			}
		}
	}
