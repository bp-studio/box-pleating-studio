...
//=================================================================
/**
 * {@link junctionTask} 負責非法重疊的計算與維護。
 */
//=================================================================
export const junctionTask = new Task(junction);

const intersection = new RRIntersection();

function junction(): void {
	for(const overlap of State.$overlaps.values()) {
		if(overlap.$valid) continue;
		const A = overlap.$a, B = overlap.$b;
		State.$junctionDiff.$add(A.id, B.id);
	...
	for(const [a, b] of State.$junctionDiff.$diff()) {
