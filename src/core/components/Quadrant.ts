...
@shrewd class Quadrant extends SheetObject {
	...
	public $makeContour(d: number): Path {
		...
		if(!pattern) {
			...
			let lead = this._findLead(junctions, r, lines, inflections);
			...
			// 那些點會干擾 PolyBool 演算法
			while(this._isInvalidHead(trace[0], r, this.q % 2 != 1)) trace.shift();

			// 底下這部份的程式碼是為了在 join 的場合中順利聯集兩組輪廓而不會在中間出現缺口而設置的，
			// 未來採用比較具宏觀性的演算法的話這段可以拿掉。
			if(start && this._outside(trace[0], r, this.q % 2 != 1)) {
			...
			if(end) {
				if(this._outside(trace[trace.length - 1], r, this.q % 2 == 1)) {
	...
	@orderedArray("qvj") private get _validJunctions(): readonly Junction[] {
		return this._flap.$validJunctions.filter(j => j.q1 == this || j.q2 == this);
	}
	...
	public $getBaseRectangle(j: Junction, base: TreeNode): Rectangle {
		let d = this.$design.tree.$dist(base, this._flap.node);
		let r = this._flap.radius;
