...
class Partitioner extends Disposable {
	...
	public *generate(): Generator<JDevice> {
		...
		if(this.overlaps.length == 1) {
			...
			if(strategy == Strategy.halfIntegral) {
				for(let g of this.halfKamiya(o, j.sx)) yield { gadgets: [g] };
			} if(strategy == Strategy.universal) {
				for(let g of this.universalGPS(o, j.sx)) yield { gadgets: [g] };
			} else {
	...
	private *universalGPS(o: JOverlap, sx: number): Generator<JGadget> {
		let d = 2, found = false;
		while(!found) {
			let bigO = clone(o);
			bigO.ox *= d; bigO.oy *= d;
	...
	private *halfKamiya(o: JOverlap, sx: number): Generator<JGadget> {
