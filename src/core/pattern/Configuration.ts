...
@shrewd class Configuration extends Store<JPattern, Pattern> implements ISerializable<JConfiguration> {
	...
	constructor(set: Repository, config: JConfiguration, seed?: JPattern) {
		...
		this.repository = set;
		this.seed = seed;
		if(seed) this.seedSignature = Pattern.getSignature(seed);
	...
	private *generate(): Generator<JPattern> {
		if(this.seed) yield this.seed;

		// 過濾掉跟存檔一樣的 Pattern
		let filter = (pattern: JPattern) => !this.seedSignature || this.seedSignature != Pattern.getSignature(pattern);
		yield* GeneratorUtil.filter(this.search([]), filter);
	}

	private *search(devices: JDevice[], depth = 0): Generator<GPattern> {
	...
	private makePattern(prototypes: JDevice[]): GPattern | null {
	...
	private getRelativeDelta(j1: JJunction, j2: JJunction, g: Gadget) {
		let oriented = j1.c[0].e == j2.c[0].e;
