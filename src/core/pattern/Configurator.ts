...
//////////////////////////////////////////////////////////////////
/**
 * `Configurator` 負責針對一個 `Repository` 搜尋 `Configuration`。
 */
//////////////////////////////////////////////////////////////////
...
class Configurator {
	...
	private readonly seed?: JConfiguration;

	/** 種子 Configuration 的簽章，用來作為快速檢查 */
	private seedSignature?: string;

	/** 種子的 Pattern */
	private readonly pattern?: JPattern;

	constructor(repo: Repository, option?: JStretch) {
		this.repo = repo;
		this.seed = option?.configuration;
		this.seedSignature = JSON.stringify(this.seed); // undefined 序列化的結果還是 undefined
	...
	public *generate(callback: Action): Generator<Configuration> {
		...
		let filter = (config: Configuration) => !this.seedSignature || this.seedSignature != JSON.stringify(config);
	...
	/** 搜尋單一分割 */
	private *searchSingleGadget(j: JJunction, strategy?: Strategy): Generator<Configuration> {
	...
	/** 搜尋雙切割接力 */
	private *searchDoubleRelay(j: JJunction, index: number): Generator<Configuration> {
	...
	/** 搜尋 3 Flap 接力 */
	private *searchThreeFlapRelay(junctions: readonly [JJunction, JJunction], strategy?: Strategy): Generator<Configuration> {
	...
	/** 搜尋 3-Flap Join */
	private *searchThreeFlapJoin(junctions: readonly [JJunction, JJunction], strategy?: Strategy): Generator<Configuration> {
	...
	private *searchThreeFlapRelayJoin(junctions: readonly [JJunction, JJunction], strategy?: Strategy): Generator<Configuration> {
		let [o1, o2] = junctions.map((j, i) => ConfigUtil.toOverlap(j, i));
