	...
	<div ref="el" class="modal fade">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content mx-4">
				...
				<div v-if="support" class="modal-body">
					<div v-if="state.stage == Stage.stopped">
						...
						<div v-if="studioService.mpSupported" class="row">
					...
					<OptProgress
						v-else-if="state.stage == Stage.initializing"
						:value="state.minor"
						:max="1"
						no-skip
						percentage
					...
					<OptProgress
						v-else-if="state.stage == Stage.start"
						:value="0"
						:max="1"
						no-skip
						percentage
					...
					<OptProgress
						v-else-if="state.stage == Stage.candidate"
						:value="state.minor"
						:max="state.major"
						percentage
					>
					...
					<div v-else-if="state.stage == Stage.continuous">
						<OptProgress
							v-if="options.layout == 'random'"
							:value="randomOverallProgress"
							:max="options.random * MAX_STEP"
							percentage
						>
					...
					<OptProgress
						v-else-if="state.stage == Stage.pack"
						:value="state.minor"
						:max="packTransform(200)"
						no-skip
						percentage
					>
						Pre-solving...
					...
					<div
						v-else-if="state.stage == Stage.error"
				...
				<div v-else class="modal-body">{{ $t("plugin.optimizer.unsupported") }}</div>
				<div class="modal-footer">
					<button
						...
						@click="state.stage = Stage.stopped"
					>
						{{ $t("keyword.close") }}
					</button>
					<button
						...
						:disabled="state.running || state.stage == Stage.error"
						@click="run"
					>
