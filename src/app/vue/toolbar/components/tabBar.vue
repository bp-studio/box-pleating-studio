<template>
	<div class="flex-grow-1 tab-container" role="tablist" @wheel.passive="tabWheel($event)">
		<SlickList
			v-bind="slickAttrs()"
			id="divTab"
			v-model:list="Workspace.ids.value"
			lock-axis="x"
			axis="x"
			:class="{ 'hide': !Workspace.ids.value.length }"
			@sort-start="setCursor('ew-resize')"
			@sort-end="setCursor('default')"
		>
			<SlickItem
				v-for="(id, i) in Workspace.ids.value"
				:key="id"
				:index="i"
				class="tab"
