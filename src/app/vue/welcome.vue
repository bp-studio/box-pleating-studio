<template>
	<div id="divWelcome" class="viewport" v-show="!Studio.project">
		<div class="d-flex flex-column h-100">
			<div class="p-3 p-md-4 p-lg-5 flex-grow-1">
					...
					<div class="row justify-content-center flex-grow-0">
						...
						<div class="browser-only col-12 col-lg-10 col-xl-8">
							<div v-if="(installAvailable || prompt || ios) && lcpReady && state == installState.uninstalled">
								...
								<button v-else-if="!nativeMode" class="btn btn-primary" @click="install"
										v-t="'welcome.install.bt'"></button>
							...
							<div v-if="state == installState.installed">
								...
								<a class="btn btn-primary" rel="noopener" :href="origin" target="_blank"
								   v-t="'welcome.install.open'"></a>
							</div>
						</div>
					</div>
...
<script setup lang="ts">
	...
	import { lcpReady, asyncComp } from "app/misc/phase";
	...
	// This part is especially time-consuming, so we separate it
	const Inner = asyncComp(() => import("./welcome-inner.vue"), true);
