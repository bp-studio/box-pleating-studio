...
vector<Circle> make_circles(const Vec &vec, const Hierarchy &hierarchy, const Hierarchy &last_hierarchy) {
	...
	for(const auto &f: hierarchy.flaps) {
		const auto *parent = hierarchy.get_parent(f.id);
		if(parent) {
			int n = find_index_by_id(last_hierarchy.flaps, parent->id);
			circles.emplace_back(vec[n * 2], vec[n * 2 + 1], parent->radius * vec.back());
		} else {
