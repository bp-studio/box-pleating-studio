...
Vue.component('dropdown', { render() { with (this) {
    ...
    } }, data() { return { bt: undefined, dropdown: undefined }; }, props: { icon: String, title: String, notify: Boolean }, computed: { ready() { return core.initialized; } }, methods: { async hide() {
            await core.libReady;
            this.dropdown.hide();
        }, mouseenter() {
...
Vue.component('filemenu', { render() { with (this) {
    ...
    } }, mixins: [BaseComponent], computed: { core() { return core; }, jsonFile() {
            return !this.design ?
                { name: "", content: () => "" } :
        ...
        document.body.addEventListener("keydown", e => {
            ...
            if (e.ctrlKey && ["o", "s", "p"].includes(e.key)) {
                ...
                if (e.key == "s" && core.design)
                    this.$refs.bps.download();
                if (e.key == "p" && core.design)
                    window.print();
            }
        });
    } });
